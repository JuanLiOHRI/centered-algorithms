...
From above, it confirms that when doing a horizontal shift, all $\beta$s except for the intercept remain unaffected. Therefore, the current task becomes to find a value $\tilde{x}$ that $\tilde{y}=\beta_0+\sum_{j=1}^{k-1}{\beta_{rcs_j}\tilde{x}_{rcs_j}}= \overline{y}$ (_i.e._ the "root" below), and use it as the offset.
...
Note here, the intercept is not exactly equal to $\overline{y}$, but the fitted value when `x == root.income`, _i.e._ `x-root.income == 0` is equal to $\overline{y}$.
