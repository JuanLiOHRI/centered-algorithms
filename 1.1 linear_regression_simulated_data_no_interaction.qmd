...
One thing to clarify is that, in the scenario when there is data of only one variable (say `x2 = cat1`), and missing for the other (`x3`, and imputed with mean value of `x2`), the predicted `y` is not equal to `mean(y)` when `x2 = cat1`, but rather to `y` when `x2 = cat1` and `x3 = mean(x3)`.
...
paste0("Mean outcome if `x2 == cat1`: ", mean(data$y[which(data$x2 == "cat1")]))
paste0("Predicted value: ", predict(fit, newdata = df))
```
