...
offset <- coef(fit.c)[1] - y_mean_logit # DISCUSSION
```
...
test_means_test$pred.c <- predict(fit.c.2, newdata = test_means_test, type = "response")
...
roc.c.2 <- roc(test$target, test_means_test$pred.c, levels=c(0, 1), na.rm=TRUE, ci=TRUE) 
...
```{r}
res <- calibration(test_means_test$pred.c, test_means_test$target, package = "rms")
res
```
