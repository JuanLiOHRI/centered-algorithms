...
## Create the predictors in the **development** dataset
...
## Create the predictors in the **external** dataset

Both distribution and summary stats can be different. For simplicity, I'll change ratios of `x1` and `x2`, and switch `x3`to `x6`.
...
ggplot(data_cat, aes(value)) +
  geom_bar() +
  facet_grid(cols = vars(type), rows = vars(var), scales = "free") +
...
Follow the same orders, with some **Note**:
