...
source("backup/util.rcs.R", echo = TRUE)
...
fit <- lm(y ~ rcs(x4, k_x4), data = data)
coef(fit)
root.x4 <- root.search(fit, data, c("x4"), k_x4, mean(data$y))$x4

# the "centered" x4
data <- data %>% mutate(x4_C1 = x4 - root.x4)
...
ggplot(data, aes(x4, y)) +
  ...
  geom_point(aes(x4_C1, y), color = "forestgreen", alpha = 0.1) +
  geom_smooth(
    aes(x4_C1, y),
    method = "lm",
    formula = y ~ rcs(x, k_x4),
...
```{r}
fit.C2 <- lm(y ~ rcs(x4_C1, k_x4), data = data)
coef(fit.C2)

# prediction using the centered algorithm when x = 0
paste0("Mean outcome in the dataset: ", mean(data$y))
predict(fit.C2, newdata = data.frame(x4_C1 = 0))
```

### A different method, treat rcs as (k-1) variables
...
# --- linear regression model on "x4_rcs_1", "x4_rcs_2", "x4_rcs_3" ---
...
1.  In order to find the root, one needs to first fit a regression using the **centered** categorical variable, and then use `root.search` to find the root.
...
# --- linear regression model on centered categotical variables and centered `rcs(x4_C2, k_x4)` ---
paste0("Mean outcome in the dataset: ", mean(data$y))
fit <- lm(y ~ x2_cat2_C + x2_cat3_C + rcs(x4_C2, k_x4), data = data)
coef(fit)
predict(fit, newdata = data.frame(x2_cat2_C = 0, x2_cat3_C = 0, x4_C2 = 0))
```
...
df_h <- data.frame(
  x = rep(range(data$x4), 2),
  y = rep(range(data$x6), each = 2),
  z = mean(data$y)
) # df for the horizontal plane
...
# finding the root for x4
res <- root.search(fit, data, c("x4", "x6"), c(k_x4, k_x6), mean(data$y))

# --- Step 3: "centering" x4 and x6 ---
data <- data %>%
  mutate(x4_C4 = x4 - res$x4, x6_C4 = x6 - res$x6)
...
# --- Step 1: model with centered variables without rcs, and original variable with rcs ---
fit <- lm(
  y ~ x1_C + x2_cat2_C + x2_cat3_C + x3_C + rcs(x4, k_x4) + x5_C + rcs(x6, k_x6),
  data = data
)
coef(fit)
...
# --- Step 3: "centering" x4 and x6 ---
data <- data %>%
  mutate(x4_C5 = x4 - res$x4, x6_C5 = x6 - res$x6)

# --- Step 3: linear regression model on all centered variable ---
paste0("Mean outcome in the dataset: ", mean(data$y))
fit <- lm(
  y ~
    x1_C +
