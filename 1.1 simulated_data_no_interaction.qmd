...
__Note:__ this is a newer version where treat rcs as (k-1) independent variables. For the original idea that to find the root as the offest instead of `x_mean`, please see the .qmd file with the same file name in the `backup` folder.
...
# visualization
fit <- lm(y ~ x4_rcs_1_C + x4_rcs_2_C + x4_rcs_3_C, data = data)
fit.df <- data %>% select(y, x4_rcs_1_C, x4_rcs_2_C, x4_rcs_3_C)
fit.df$pred <- predict(fit)
names(fit.df)[which(names(fit.df) == "x4_rcs_1_C")] <- "x4"
...
ggplot(data, aes(x4, y)) +
  ...
  #geom_point(aes_auto(fit.df), data=fit.df, color = "forestgreen") +
  geom_path(data=fit.df %>% arrange(x4), aes(x = x4, y = pred), color = "forestgreen") +
  theme_bw()
```
