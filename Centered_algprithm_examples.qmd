...
However, the offset here is a constant given $\overline{x}$ and knot locations: $C_{offset} = (y|x=\overline{x})-\overline{y} = \beta_0+\sum_{j=1}^{k-1}{\beta_{rcs_j}(x_{rcs_j}|x=\overline{x})}-\overline{y}$.

```{r}
y_mean <- mean(data$prestige)

x_mean <- mean(data$income)
knots <- attributes(rcs.fit)$parms # knot locations
coefs <- coef(fit)
(offset <- offset_rcs(y_mean, x_mean, knots, coefs))

x_mean_C <- mean(data$income_C)
knots_C <- attributes(rcs.fit.C)$parms # knot locations
coefs_C <- coef(fit.C)
(offset_C <- offset_rcs(y_mean, x_mean_C, knots_C, coefs_C))
paste0("The predicted y when x-x_mean = 0 minus the offset: ", predict(fit.C, newdata = data.frame(income_C=0)) - offset_C) 
paste0("Mean prestige score in the dataset: ", mean(data$prestige))
```

### **Experiment:** when the predictor is normal distributed

**Conclusion:** even when the predictor is normal distributed: (1) `mean(x)` and `median(x)` are close, but not identical; (2) `y ~ rcs(x-mean(x), k)` rarely across `mean(y)` when `x-mean(x)=0`.
...
expriment <- function(seed) {
  ...
  p1 <- ggplot(df, aes(x1, y)) + 
    geom_point() +
    geom_smooth(method = "lm", formula = y ~ x)+
    geom_hline(yintercept = mean(df$y), color = "red", linetype = "dashed") +
    annotate("label", x = 25, y = mean(df$y), label = "mean(df$y)", size = unit(3, "pt")) + 
    geom_vline(xintercept = mean(df$x1), color = "red", linetype = "dashed")+
    annotate("label", x = mean(df$x1), y = 900, label = "mean(df$x1)", size = unit(3, "pt")) +
    theme_bw()+
    labs(title = paste0("seed = ", seed))+
    xlim(20,80)
  
  rcs.fit <- rcs(df$x1, 5)
  knots <- attributes(rcs.fit)$parms
  p2 <- ggplot(df, aes(x1, y)) + 
    geom_point() +
    ...
    geom_vline(xintercept = mean(df$x1), color = "red", linetype = "dashed")+
    annotate("label", x = mean(df$x1), y = 900, label = "mean(df$x1)", size = unit(3, "pt")) +
    geom_vline(xintercept = knots, color = "black", linetype = "dashed")+
    theme_bw() +
    labs(title = paste0("seed = ", seed))+
    xlim(20,80)
...
# --- linear regression model on centered variable `income_C` and `education_C` ---
fit <- lm(prestige ~ rcs(income_C,k) + rcs(education_C,k), data = data)
coef(fit)
```
...
# --- linear regression model on centered variable `income_C` ---
fit_C <- lm(prestige ~ rcs(income_C,k) + type_prof_C + type_wc_C, data = data)
coef(fit_C)
```
