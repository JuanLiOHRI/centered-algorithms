...
# =========================================================
#' Modify `brier.cox` to work with Fine-Gray models
#'
#' @param pred.prob  Vector of the predicted event probabilities
#' @param fgstart Vector of the fgstart variable
#' @param fgstop Vector of the fgstop variable
#' @param fgstatus Vector of the fgstatus variable
#' @param fgwt Vector of the fgwt variable
#' For other parameters, see `survival::brier`
#' @return     Dataframe of metrics
#' @export
brier.fg <- function(
  ...
  Call <- match.call()
  Y <- Surv(fgstart, fgstop, fgstatus) # Y <- mf[[1]]
  # JL: Note for fine-gray attr(Y, "type") = "counting"
  # if (!is.Surv(Y)) {
  #   stop("response must be a Surv object")
  # }
  # type <- attr(Y, "type")
  ...
  ntime <- length(times)
  if (ties) {
    mindiff1 <- min(diff(sort(unique(dtime1))))
    dtime1 <- dtime1 + ifelse(dstat == 0, mindiff1 / 2, 0)

    mindiff2 <- min(diff(sort(unique(dtime2))))
    dtime2 <- dtime2 + ifelse(dstat == 0, mindiff2 / 2, 0)
  }
  c0 <- survfit0(survfit(Surv(dtime1, dtime2, 1 - dstat) ~ 1, weights = casewt))
  b0 <- b1 <- matrix(0, nrow = n, ncol = ntime)
  casewt <- casewt / sum(casewt)
  brier <- matrix(0, ntime, 2)
  eff.n <- double(ntime)
